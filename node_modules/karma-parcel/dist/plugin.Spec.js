...
describe("plugin", () => {
    ...
    it("exposes a ParcelPlugin", () => {
        assert.ok(plugin_1.ParcelPlugin);
    });
    it("exposes a factory", () => {
        assert.ok(plugin_1.createParcelPlugin);
    });
    describe(plugin_1.ParcelPlugin.name, () => {
        describe("workspace()", () => {
            const cwd = os.tmpdir();
            beforeEach(() => {
            ...
            it("returns a workspace", () => {
                const plugin = new plugin_1.ParcelPlugin(logger, karmaConf, emitterStub());
        ...
        describe("addFile", () => {
            it("writes the file to the underlying entry file", () => {
                ...
                sinon.stub(process, "cwd").returns(cwd);
                const add = sinon.stub(files_1.EntryFile.prototype, "add").resolves();
                const plugin = new plugin_1.ParcelPlugin(logger, karmaConf, emitterStub());
                return plugin
                    .addFile({
        ...
        describe("middleware", () => {
            ...
            beforeEach(() => {
                ...
                sinon.stub(process, "cwd").returns(cwd);
                plugin = new plugin_1.ParcelPlugin(logger, karmaConf, emitterStub());
            ...
            describe("if url contains .karma-parcel", () => {
                ...
                it("creates a bundler with the entry.js as the entry point", () => {
                    plugin.middleware(req, resp, sinon.stub());
                    sinon.assert.calledWithMatch(createBundler, path.join(cwd, ".karma-parcel", "entry.js"));
                });
                ...
                it("creates the bundler with the given karmaKonf.parcelConfig", () => {
                    ...
                    plugin.middleware(req, resp, sinon.stub());
                    sinon.assert.calledWithMatch(createBundler, sinon.match.any, karmaConf.parcelConfig);
                });
